...
const walkers = {
  ArrayExpression(node, state, visit) {
    ...
    for (const element of node.elements) {
      ...
      const value = identifierToVarRef(state.lastChild);
      array.push(value);
    }
  ...
  BinaryExpression(node, state, visit) {
    visit(node.left, state, node.left.type);
    const left = identifierToVarRef(state.lastChild);
    visit(node.right, state, node.right.type);
    const right = identifierToVarRef(state.lastChild);
  ...
  CallExpression(node, state, visit) {
    ...
    const args = node.arguments.map((a) => {
      ...
      return identifierToVarRef(state.lastChild);
    });
  ...
  ExpressionStatement(node, state, visit) {
    ...
    if (node.expression.type === 'AssignmentExpression') {
      ...
      const reference = identifierToVarRef(expr.left);
      state.lastChild = new SetDirectiveNode(reference, expr.right);
    } else {
      // TODO(cjihrig): Use a temp variable for all throw away values.
      const reference = identifierToVarRef(new IdentifierNode('discard'));
  ...
  SwitchStatement(node, state, visit) {
    ...
    const matched = identifierToVarRef(new IdentifierNode('matched'));
    const fallthrough = identifierToVarRef(new IdentifierNode('fallthrough'));
    const discriminant = identifierToVarRef(new IdentifierNode('discriminant'));
...
function identifierToVarRef(identifier) {
  // Convenience function to create an Identifier in a VariableReference.
